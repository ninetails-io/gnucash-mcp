...
Three things this server stored in a private shape only it could read are now written the way GnuCash writes them (below: schedule recipes, the invoice link, budget signs). **One write converts the whole book:** the first schedule, budget, or business write after upgrading runs every conversion at once, posts nothing, reports what it converted (`templates_migrated`, `invoice_links_migrated`, `book_stamped`, `book_scrubbed`) and writes it to the audit log. A no-change `update_scheduled_transaction` on any schedule is the deliberate one-call version. Do it before your next GnuCash desktop session — until then, opening a server-made schedule in desktop's Scheduled Transaction Editor crashes GnuCash 5.12, and the dashboard warns while any remain. Reads never write.

### Fixed
- The demo-book continuation's frozen-prefix check counts ledger transactions only. Since the native-template conversion merged, its schedule step added one template row per converted schedule, dated inside the prefix, and every CI bundle build failed on "PREFIX VIOLATION". Template rows are not activity.
- **Audit and debug logs open the day's file by path on every entry.** The writer used to hold one descriptor for the life of the process, so a log file renamed or deleted underneath it kept receiving entries in an unlinked inode while `get_audit_log` reported no log for the day, and a server left running past midnight kept writing into yesterday's file. Both are gone; a removed file comes back with its header and owner-only permissions on the next write.
- **The invoice-link conversion also matches the spelling GnuCash desktop writes back.** Desktop re-serializes every KVP child under its frame's full path, so a book saved in desktop since 1.2 carries `gncInvoice/invoice` where the server wrote `invoice`; the sweep matched only the bare form and left Jump to Invoice greyed out on every old posting (found on the MariaDB loop: 108 of 108 links). Both spellings rename now.
- A database book announces a server (re)start like a file book does, and its audit header names the masked connection URI instead of the storage key (`gnucash.gnucash`).
- **Lot open/closed state read the way GnuCash defines it.** `lots.is_closed` is a tri-state (`gnc-lot.cpp`: `LOT_CLOSED_UNKNOWN (-1)`, recomputed from the balance whenever desktop touches the lot; `1` cached for a zero balance). The server believed `-1` meant closed, wrote it to close lots, and read it as closed — so after any GnuCash desktop session every open lot desktop had touched vanished from `list_lots` and reported `is_closed: true` (63 of 117 lots on a desktop-saved copy of the demo book). One reader now, `_lot_is_closed`, ported from `gnc_lot_is_closed`; closing writes `1`. `close_lot` refuses a lot that still holds a balance, since GnuCash would reopen it on its next recompute.
- **Output order comes from the data, not the storage engine.** SQLite returns rows in insertion order and InnoDB in primary-key order, and several renderers never sorted: a transaction's splits (now GnuCash's own `xaccTransSortSplits` rule — debits first — then account path), same-day transactions (entry time, then GUID), the schedule list (next due, then name), the upcoming list, the dashboard's overdue and stale-price previews, and lots (acquisition date). Every read surface now renders a MySQL copy of a book identically to the file, verified call by call on the demo book.
...
- **Scheduled transactions store their recipe the way GnuCash does** — a template transaction on the schedule's template account, one zero-value split per leg carrying Split.cpp's `sched-xaction` slots (target account GUID, credit/debit formula and numeric) — so GnuCash desktop's Since-Last-Run posts what this server scheduled, and a schedule made in desktop can be instantiated here. Before, the recipe lived in a private `splits-json` slot: desktop advanced our schedules with nothing posted, and desktop-made schedules had no recipe here. Existing schedules migrate on the first schedule write of any kind — every legacy recipe in the book converts at once, nothing posted for the others (`templates_migrated` in the response and on the audit line); a no-change `update_scheduled_transaction` is the deliberate one-call conversion, and the dashboard counts what's left. **Upgrading from 1.2–1.4.4 with schedules made through this server: run that conversion before the next GnuCash desktop session.** Until then, opening any of those schedules in GnuCash 5.12's Scheduled Transaction Editor crashes GnuCash (confirmed on a real book, 2026-09-10), and one OK on desktop's Since-Last-Run silently advances every un-converted schedule with nothing posted. On 1.4.x without upgrading: don't open server-made schedules in the editor, and Cancel Since-Last-Run. The dashboard warns while any remain. Reads never write. Instantiated transactions carry `from-sched-xaction`, as desktop's do. A desktop cross-commodity leg takes its rate from `book.prices` at the instance date or refuses naming the leg; a formula with variables is refused naming the formula. The template account is named by the schedule's GUID on GnuCash's `template` pseudo-commodity (created if the book lacks it), matching `xaccSchedXactionInit`.
- Occurrence math reads the recurrence rows, as desktop does: `Recurrence.cpp`'s next-instance routine is ported verbatim, so the anchor is the row's period start (not `start_date`), all eight period types and the weekend adjustment are honored, and a composite schedule's next occurrence is the earliest across its rows. `frequency` is derived from the rows (`every 6 months`, `end of month, weekends forward`, `composite (2 rules)`). A desktop schedule "start 9 Sep, monthly on the 15th" now posts on the 15th; before, it posted on the 9th and desktop posted the 15th again. Deleting a schedule removes every recurrence row (a composite used to leave one orphaned). The dashboard's Scheduled line counts schedules still on the legacy recipe.
- **The link from a posting transaction and its lot back to the invoice is written under GnuCash's key** — a `gncInvoice` frame with a `gncInvoice/invoice-guid` child (`gnc-engine.h`, verbatim, pinned by a test). Since 1.2 the child was named `invoice`, which desktop never reads, so "Jump to Invoice" from the register, the lot viewer's invoice title, and Process Payment's open-lot list could not find a server-posted document. Every business write (post, pay, unpost, apply, delete) renames the book's pre-1.5 links on the way — `invoice_links_migrated` in the response and on the audit line, nothing posted. Lot titles and posting-split actions now carry GnuCash's type string per document (`Bill 000012`, `Expense`, `Credit Note`) instead of `Invoice` for everything.
- Deleting a transaction or a schedule strips GUID-valued slots by raw SQL before the ORM delete (`_strip_guid_slots`): piecash's `SlotGUID` cascade would otherwise delete every slot of the entity the slot points at — the target account's for a template split, the schedule's for a stamped instance, the invoice's for a posting transaction (the credit-note incident).
- Budget amounts follow GnuCash 3.8+'s natural-sign storage: an income, liability, payable, equity, or credit-card target is stored negative and the book carries GnuCash's `Use natural signs in budget amounts` feature stamp, so GnuCash desktop displays what this server wrote and this server reads what GnuCash wrote. The tool surface is unchanged — amounts are magnitudes for every account type. Before, every type was stored as a magnitude: an income target read as `-5,000 / 0%` once GnuCash scrubbed the book on open, and displayed as -5,000 in GnuCash when written here. Un-stamped books are read through the same heuristic GnuCash's open-time scrub uses, and the first budget write performs that scrub and stamps the book. Expense-only budgets (every sample book) are byte-identical before and after.

...
- `get_budget_report` keeps income and expense targets on their own sides. A budget with both ends in INCOME / EXPENSES / NET lines (verbose: `subtotals` per side, `totals` is the net with `basis` named) instead of one TOTAL that added a 5,000 income target to 300 of expenses. A single-side budget is unchanged. The dashboard's Budget headline paces expense targets only.
